...
uint16_t Gpu3D::rgb5ToRgb6(uint16_t color)
{
    uint8_t r = ((color >>  0) & 0x1F) << 1; if (r > 0) r++;
    uint8_t g = ((color >>  5) & 0x1F) << 1; if (g > 0) g++;
    uint8_t b = ((color >> 10) & 0x1F) << 1; if (b > 0) b++;
    return (b << 12) | (g << 6) | r;
}

uint16_t rgb5ToRgb5psp(uint16_t color)
{
    uint8_t r = ((color >>  0) & 0x1F) << 1; if (r > 0) r++;
    uint8_t g = ((color >>  5) & 0x1F) << 1; if (g > 0) g++;
    uint8_t b = ((color >> 10) & 0x1F) << 1; if (b > 0) b++;
...
Matrix Gpu3D::multiply(Matrix *mtx1, Matrix *mtx2)
{
    ...
    __asm__ volatile (
		...
		: "=m"(matrix.data) : "m"(mtx1->data), "m"(mtx2->data)
		);
...
bool Gpu3D::clipPolygon(Vertex *unclipped, Vertex *clipped, int *size)
{
    ...
    for (int i = 0; i < 6; i++)
    {
        ...
        *size = 0;
        ...
        if (i < 5) memcpy_vfpu(unclipped, clipped, *size * sizeof(Vertex));
    }
...
void Gpu3D::colorCmd(uint32_t param)
{
    ...
    savedVertex.color = rgb5ToRgb5psp(param);
}
...
void Gpu3D::difAmbCmd(uint32_t param)
{
    ...
    if (param & BIT(15))
        savedVertex.color = rgb5ToRgb5psp(param >>  0);
}
