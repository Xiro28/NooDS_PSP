...
void Gpu2D::drawGbaScanline(int line)
{
    ...
    for (int i = 0; i < 240; i++)
    {
        ...
        if (dispCnt & 0xE000) // Windows enabled
        {
            ...
            else if ((dispCnt & BIT(15)) && (framebuffer[line * 256 + i] & BIT(15)))
        ...
        uint16_t pixel = U8TO16(palette, 0), pixel2 = pixel;
        ...
        framebuffer[(line + 16) * 256 + 8 + i] = (pixel);
    }
    ...
    if (line == 159)
    {
        ...
        for (int y = 0; y < 192; y++)
        {
            for (int x = 0; x < 256; x++)
            {
                ...
                framebuffer[y * 256 + x] = (core->memory.read<uint16_t>(0, base + (y * 256 + x) * 2));
            }
        }
    }
}
...
void Gpu2D::drawScanline(int line)
{
    ...
    for (int i = 0; i < 256; i++)
    {
        ...
        /*if ((*pixel & BIT(15))) *pixel = (*pixel);
        if ((pixel2 & BIT(15))) pixel2 = (pixel2);*/
    ...
    switch ((dispCnt & 0x00030000) >> 16) // Display mode
    {
        ...
        case 1: // Layers
        {
            memcpy_vfpu(&framebuffer[line * 256], layers[5], 256 * sizeof(uint16_t));
            break;
        }
...
void Gpu2D::drawText(int bg, int line)
{
    ...
    {
        memcpy_vfpu(layers[bg], core->gpu3DRenderer.getLine(line), 256 * sizeof(uint32_t));
        return;
    }
