...
volatile uint16_t _vCount = 0;

int _drawThreaded(int data)
{
    Core *core = (Core*)data;
    ...
    while (1)
    {
        ...
        //while(PSP_UC(drawing) != 1) asm("nop");

        uint16_t _vc =  PSP_UC(_vCount);

        PSP_UC(drawing) = 2;

        // Draw the scanlines
        core->gpu2D[0].drawScanline(_vc);
        
        // Draw engine B's scanline if it hasn't started yet
        if (PSP_UC(drawing) == 2){
            PSP_UC(drawing) = 3;
            core->gpu2D[1].drawScanline(_vc);
        }
...
Gpu::Gpu(Core *core): core(core)
{
    ...
    //thread = sceKernelCreateThread("_GPU",&_drawThreaded,0x18,0x10000, THREAD_ATTR_USER, NULL);
}
...
void Gpu::scanline256()
{
    ...
    /*if (firsttime){
        J_EXECUTE_ME_ONCE(_drawThreaded, (int)core);
        firsttime = false;
    }*/
    ...
    if (vCount < 192)
    {
        if (thread)
        {
            ...
            //while (drawing.load() == 1) sceKernelWaitThreadEnd(thread, &timeout);

            //sceKernelWakeupThread(thread);

            switch (PSP_UC(drawing))
            {
                case 2:
                ...
                case 3:
                    ...
                    while(PSP_UC(drawing) != 0) asm("nop");
                break;
            }
        }
        else
        {
                // Draw visible scanlines
                if (flipflop) core->gpu2D[0].drawScanline(vCount);
                else          core->gpu2D[1].drawScanline(vCount);
        }
        ...
        if (displayCapture)
        {
            ...
            switch ((dispCapCnt & 0x60000000) >> 29) // Capture source
            {
                case 0: // Source A
                {
                    ...
                    uint32_t *source = (dispCapCnt & BIT(24)) ? (uint32_t*)core->gpu3DRenderer.getLine(vCount) : (uint32_t*)core->gpu2D[0].getRawLine();

                    // Copy a scanline to memory
                    for (int i = 0; i < width; i++)
                        core->memory.write<uint16_t>(0, base + (writeOffset + i * 2) % 0x20000, (source[i]));
